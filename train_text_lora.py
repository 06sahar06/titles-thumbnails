...
def sanity_check(model, tokenizer, device: torch.device) -> None:
    model.eval()
...
def compute_class_weights(dataset, label_column: str, device: torch.device) -> torch.Tensor:
    ...
    total = len(df)
    w0 = total / (2 * counts[0])  # weight for class 0 (non-clickbait)
    w1 = total / (2 * counts[1])  # weight for class 1 (clickbait)
    print(f"Class counts: non-clickbait={counts[0]}, clickbait={counts[1]}")
    print(f"Class weights: non-clickbait={w0:.4f}, clickbait={w1:.4f}")
...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            print("Warning: TrainingArguments fallback to minimal config — best-model tracking disabled.")
            training_arguments = TrainingArguments(
                output_dir=str(output_dir),
                num_train_epochs=args.epochs,
                per_device_train_batch_size=args.batch_size,
    ...
    trainer_kwargs = {
        ...
        "class_weights": class_weights,  # passed to WeightedTrainer
    }
    ...
    # Always use WeightedTrainer — it handles class imbalance via loss weighting
    try:
        trainer = WeightedTrainer(**trainer_kwargs)
    except TypeError:
        print("Warning: WeightedTrainer init failed with provided kwargs; retrying with minimal args.")
        minimal_kwargs = {
            k: trainer_kwargs[k]
            for k in ("model", "args", "train_dataset", "eval_dataset", "class_weights")
        }
        trainer = WeightedTrainer(**minimal_kwargs)
    ...
    test_logits = np.asarray(test_output.predictions[0] if isinstance(test_output.predictions, tuple) else test_output.predictions)
    test_labels = np.asarray(test_output.label_ids)
    test_metrics = compute_binary_metrics(test_logits, test_labels)
    append_metrics_record(
        results_path,
        {
